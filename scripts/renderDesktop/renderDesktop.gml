function renderDesktop(){
    if (layer_get_visible(global.email_buttonlayer)) {
        ...
        if (variable_global_exists("viewed_email")) {
            ...
            draw_text(972, 248, global.viewed_email[$ "title"]);
            draw_set_font(mediumGUI);
            draw_text_ext(972, 320, global.viewed_email[$ "content"], 38, 400);
        }
    }
    if (layer_get_visible(global.shop_buttonlayer)) {
        if (variable_global_exists("viewed_category") && global.viewed_category != undefined) {
            title_x = 580;
            tilemap_set_at_pixel(global.gui_tilemap[2], 212, 580, 360);
            ...
            title_x = 640;
            tilemap_set_at_pixel(global.gui_tilemap[2], 0, 580, 360);
            for (i=global.y_offset+1;i<(7 + global.y_offset);i++) {
                product_category = global.shopdata[$ (struct_get_names(global.shopdata)[i])]
                title_y = 336 + (i - 1) * 48 - global.y_offset * 48;
                tilemap_set_at_pixel(global.gui_tilemap[3], product_category[$ "icon"], 580, title_y);
        ...
        if (variable_global_exists("viewed_product") && global.viewed_product != undefined) {
            ...
            draw_text_ext(1056, 432, global.viewed_product[$ "description"], 34, 344); 
